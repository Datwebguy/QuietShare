...
import (
	...
	"extension-scaffold/tools/pkg/contracts/quietshare"
...
func DeployInstructionSender(s *support.Support) (common.Address, *quietshare.QuietShareInstructionSender, error) {
	...
	address, tx, contract, err := quietshare.DeployQuietShareInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := quietshare.NewQuietShareInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := quietshare.QuietShareInstructionSenderMetaData.GetAbi()
			if parsed != nil {
...
func SendRecordDeposit(s *support.Support, instructionSenderAddress common.Address, potId [32]byte, encryptedNote []byte) (common.Hash, common.Hash, error) {
	sender, err := quietshare.NewQuietShareInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.SendRecordDeposit(opts, potId, encryptedNote)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := quietshare.QuietShareInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendRecordDeposit", potId, encryptedNote)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
...
func SendGetBalance(s *support.Support, instructionSenderAddress common.Address, potId [32]byte) (common.Hash, common.Hash, error) {
	sender, err := quietshare.NewQuietShareInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to bind contract: %s", err)
	}
	...
	tx, err := sender.SendGetBalance(opts, potId)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to send instruction: %s", err)
	}
